...
describe('E2E Payments', () => {
  ...
  it('checkout creates holds and returns checkoutUrl, then webhook succeeds and books seats', async () => {
    ...
    const seatList1 = await request(app).get(
      `/api/sections/${section.id}/seats`,
    );
    expect(
      seatList1.body.data.find((s: any) => s.id === seatIds[0]).status,
    ).toBe('reserved');
    ...
    let tickets: any[] = [];
    for (let i = 0; i < 6; i++) {
      tickets = await AppDataSource.getRepository('Ticket' as any).find({
        where: { user: { id: customer.id } } as any,
      });
    ...
    const seatList2 = await request(app).get(
      `/api/sections/${section.id}/seats`,
    );
    expect(
      seatList2.body.data.find((s: any) => s.id === seatIds[0]).status,
    ).toBe('booked');
  ...
  it('webhook is idempotent (duplicate evt id ignored, no double booking)', async () => {
    ...
    const customer = await createTestUser({
      email: `cust2-${Date.now()}@example.com`,
    });
    const venue = await createTestVenue();
    const event = await createTestEvent(organizer.id, venue.id);
    const section = await createTestSection(event.id);
    ...
    const payments = await AppDataSource.getRepository('Payment' as any).find();
    const dupPayments = payments.filter(
      (p: any) => p.idempotencyKey === 'evt_dup_test',
    );
